...
impl FanoutBenchmarkHarness {
    pub async fn new(
        ...
        let temp_dir = TempDir::new().context("create temp dir for fanout benchmark")?;
        ...
        fs::write(
            package_dir.join("benchmark_fanout.py"),
            fixtures::BENCHMARK_FANOUT.trim_start().as_bytes(),
        )
        .context("write benchmark_fanout fixture")?;

        let user_module = "workflow_bench.benchmark_fanout".to_string();
        ...
        info!(
            ...
            "fanout benchmark harness initialized"
        );
    ...
    pub async fn run(&self, config: &FanoutBenchmarkConfig) -> Result<BenchmarkSummary> {
        self.database.reset_workflow_state().await?;

        // Build workflow input with parameters
        ...
        // Create all workflow instances using push-based scheduling
...
payload = module.FanoutBenchmarkWorkflow._build_registration_payload()
